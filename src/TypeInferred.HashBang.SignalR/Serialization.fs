[<ReflectedDefinition>]
module TypeInferred.HashBang.SignalR.Serialization
...
type JsonReader(jsonStr : string) =
    ...
    let readUpToExcl charSet areEscapedIgnored =
        let rec readUpToExcl count isEscaped =
            let nextC = tryPeek()
            match nextC with
            | Some c -> 
                let canExit = 
                    (not areEscapedIgnored 
                    || not isEscaped)
                    && charSet |> Set.contains c
                if canExit then count
                else
                    consume()
    ...
    member __.ReadUpToQuoteMark() =
        readUpToExcl quoteMarks true
        
    member __.ReadUpToSeparatorInclWhiteSpace() =
        readUpToExcl separators false
...
let precomputeDeserializerAux (precomputeDeserializer : _ -> _ Lazy) t : JsonReader -> obj =
    ...
    | StringType extract ->
        fun (jr : JsonReader) ->
            ...
            jr.MovePast '"'
            let unescaped = Uri.UnescapeDataString value
            box unescaped
...
let precomputeSerializerAux (precomputeSerializer : _ -> _ Lazy) t : obj -> StringWriter -> unit =
    ...
    | StringType extract ->
        fun obj (sb : StringWriter) ->
            let x = extract obj
            let escaped = Uri.EscapeDataString x
            sb.Append('"').Append(escaped).Append('"') |> ignore
    | Int32Type extract ->
        fun obj (sb : StringWriter) ->
            let x = extract obj
            sb.Append(x) |> ignore
