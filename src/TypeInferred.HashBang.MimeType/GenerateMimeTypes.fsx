    ...
    async {
        ...
        return 
            seq {
                ...
                yield "module MimeTypes ="
                yield "    let lookupByExtension ="
                yield "        Map [|"
                for doc, t, ext in pairs do
                    yield sprintf "            \"%s\", MimeType \"%s\"" ext t
                yield "        |]"
                yield ""
...
let generateFile() =
    async {
        ...
        let precursor = File.ReadAllText(Path.Combine(__SOURCE_DIRECTORY__, "MimeTypeHeader.fsx"))
        let fullCode = precursor + code
        File.WriteAllText(Path.Combine(__SOURCE_DIRECTORY__, "MimeType.fs"), fullCode)
