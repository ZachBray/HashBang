...
namespace TypeInferred.HashBang.Routing.ProvidedTypes
...
open TypeInferred.HashBang.Routing.Utilities
...
module internal Misc =
    ...
    let transQuotationToCode isGenerated qexprf = 
        let exprAndVars = ref None
        fun (argExprs: Quotations.Expr[]) ->
            let vars, expr =
                match !exprAndVars with
                | None -> 
                    // add let bindings for arguments to ensure that arguments will be evaluated
                    // Note: this also means we can cache the rest of the quotation building
...
type ProvidedMethod(methodName: string, parameters: ProvidedParameter list, returnType: Type) as this =
    ...
    let getInvokeCodeInternal =
        let getInvokeCodeInternalAux isGenerated =
            match invokeCode with
            | Some f ->  transQuotationToCode isGenerated f
        ...
        memoize getInvokeCodeInternalAux
    ...
    member this.GetInvokeCodeInternal isGenerated = getInvokeCodeInternal isGenerated
   // Implement overloads
...
type TypeProviderForNamespaces(namespacesAndTypes : list<(string * list<ProvidedTypeDefinition>)>) as this =
    ...
    static let parametricCache = Dictionary HashIdentity.Structural
    ...
    default this.ResolveAssembly(args) = 
        let expectedNames = 
            [
                ...
                (AssemblyName(args.Name)).Name + ".exe"
            ]
        let expectedLocationOpt = 
            probingFolders 
            |> Seq.collect (fun f -> 
                expectedNames |> Seq.map (fun expectedName ->
                    IO.Path.Combine(f, expectedName)))
